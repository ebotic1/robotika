...
void Extrusion::AddIndices(std::vector<GLuint>& indices, int start)
{
...
void Extrusion::Extrude(const std::vector<glm::vec3>& path, std::vector<GLfloat>& verts, std::vector<GLuint>& inds) // napraviti da se ponovnim pozivom funkcije extruzija nastavlja. Ovaj interni vektor ce se std::move pri povratku unistiti tako da treba napraviti nesto sto ce sacuvati gjde se stalo
{
	_steps.clean();
	constexpr float epsilon = glm::epsilon<float>() * 3;
	std::vector<GLfloat> vektor(0);
...
void Extrusion::Extrude(const std::vector<glm::vec3>& path, gui::gl::Buffer& glBuf, gui::gl::Textures::Unit TU, int Texind)
{
	int szv = glBuf.getNoOfVertices(), szi = glBuf.getNoOfIndices();
	std::vector<GLfloat> verts;
	std::vector<GLuint> inds;
	//inds.reserve(2000000);
	Extrude(path, verts, inds);
	float* vert = new float[verts.size()];
	float* poc = vert;
	for (auto x : verts)
		*(poc++) = x;
	for (auto& x : inds) 
		x += szv;
	int parameters = 3;
	if (addTextureCoord) parameters += 2;
	if (addNormals) parameters += 3;
	glBuf.appendVertices(vert, verts.size() / parameters);
	glBuf.appendIndices((gui::gl::IndexType*)(inds.data()), inds.size());
	auto cmd = glBuf.createCommand();
	//pCubeCmd->createDrawArrays(gui::gl::Primitive::Triangles, 0, _vec.size() / 3);
	cmd->createDrawElements(gui::gl::Primitive::Triangles, szi, inds.size());
	if (addTextureCoord)cmd->setTexture(TU, Texind);
	glBuf.encode(cmd);
}
